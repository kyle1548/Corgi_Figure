    ...
    def trot_loop(self, event):
        if self.fsm == "idle":
            ...
            if self.motion_init == False and accel_on == 0:
                ...
                # ]
                self.swing_landpt = [
                    self.gait.X_ref[self.ref_cnt][:3, 3].reshape(3)
                    + self.bhip[i]
                    + np.array(
                        [
                            self.state.legs[i].pose[0, 0],
            ...
            if pid_comp[0, 0] > self.gait.vx * self.dt * 18:
                pid_comp[0, 0] = self.gait.vx * self.dt * 18
            elif pid_comp[0, 0] < -self.gait.vx * self.dt * 18:
                pid_comp[0, 0] = -self.gait.vx * self.dt * 18
...
if __name__ == "__main__":
    ...
    # loadConfig(
    #     filepath="/home/guanlunlu/Desktop/Corgi_docker_ws_0617/Corgi_docker_ws/corgi_rosws/src/corgi_mpc/config/config_trot.yaml"
    # )
