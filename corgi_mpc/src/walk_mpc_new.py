...
class Corgi_mpc:
    def __init__(self, dt=0.01):
        ...
        self.ref0 = None
        self.flip_pid = VectorPIDController(np.array([0.2]), np.array([0]), np.array([0]), dt, setpoint=np.array([0]))
    ...
    def walk_loop(self, event):
        if self.fsm == "idle":
            ...
            for i in range(4):
                if legs_contact[i] == 1:
                    st_rs_.append(
                        self.bhip[i] + np.array([self.state.legs[i].pose[0, 0], 0, self.state.legs[i].pose[1, 0]]))
                else:
            ...
            b_x_f = 0
            x_tol = 0.013
            t_ = self.ref_cnt*self.dt
            if sw_idx == 0 or sw_idx == 3:
                ft1_x = st_rs_[1]
                ft3_x = st_rs_[3]
                b_x_f = ((ft1_x + ft3_x) / 2)[0]
            else:
                ft0_x = st_rs_[0]
                ft2_x = st_rs_[2]
                b_x_f = ((ft0_x + ft2_x) / 2)[0]
            
            if self.ref0 == None or if_switch_leg:
                if sw_idx == 2 or sw_idx == 3:
                    self.ref0 = b_x_f
                    self.flip_pid.integral = 0
                    
            if self.ref0 == None:
                ref_t = (-x_tol - b_x_f) * sw_duty
            else:
                ref_t = b_x_f + (-x_tol - b_x_f) * (np.fmod(t_-self.gait.T_sw, 2*self.gait.T_sw)/2*self.gait.T_sw)
            
            self.flip_pid.setpoint = ref_t
            disp_x = -1 * self.flip_pid.update(np.array([b_x_f]))[0]
            print("b_x_f, ", b_x_f)
            print("delta: ", ref_t - b_x_f)
            print("--")
            
            # Footend Position cmd generate by body pose, orientation tracking
            # translation
            self.body_pos_pid.setpoint = X_ref_rt[:3, 3].reshape(3)
            disp_xyz = self.body_pos_pid.update(self.state.pose.reshape(3)).reshape(3, 1)
            disp_xyz[0,0] = disp_x
            next_pose = self.state.pose + disp_xyz
