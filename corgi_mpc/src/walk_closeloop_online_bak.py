...
class logger:
    ...
    def update_data(self, seq, x, z, p, xd, zd, pd):
...
class Corgi_mpc:
    ...
    def stateCallback(self, data):
        ...
        # signal_z = self.gait.stance_height
        ...
        self.state.twist = np.array(
            [
                ...
                # [self.feedback_filter_vx.apply_filter(data.twist.linear.x)],
                [data.twist.linear.x],
                [data.twist.linear.y],
                [data.twist.linear.z],
            ]
    ...
    def walk_loop(self, event):
        if self.fsm == "idle":
            ...
            if closed_loop == 1 and self.close_loop_init == False:
                self.gait.p0 = self.state.pose
                self.gait.setup_reference_traj_tol()
                self.swing_landpt = [self.state.pose.reshape(3) + self.bhip[i] + \
                                     np.array([self.state.legs[i].pose[0,0], 0, self.state.legs[i].pose[1,0]]) \
            ...
            if sw_idx != -1:
                ...
                # print("sw_duty, sw_rs_next, ",sw_duty, sw_rs_next.reshape(3))
            ...
            if closed_loop == 1:
                ...
                """ self.logger.update_data(self.seq,
                                        self.state.filtered_pose[0,0],
                                        self.state.filtered_pose[2,0],
                                        p_,
                                        X_ref_rt[0, 3],
                                        X_ref_rt[2, 3],
                                        desired_zyx[1]) """
